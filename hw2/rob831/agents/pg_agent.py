...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.standardize_advantages:
            ...
            advantages /= (np.std(advantages) + 1e-5)
    ...
    def _discounted_return(self, rewards):
        ...
        # create discounted_returns
        rewards = np.array(rewards)
        discount = np.power(self.gamma, np.arange(len(rewards)))
        discounted_returns = np.sum(discount * rewards)
        discounted_returns = np.ones_like(rewards) * discounted_returns
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for i in range(len(rewards)-2):
            discounted_cumsums[len(rewards)-2-i] = discounted_cumsums[len(rewards)-1-i] * self.gamma + rewards[len(rewards)-2-i]
